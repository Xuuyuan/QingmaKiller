...
- 网络题库内置于 `tiku.py`(搜题逻辑移植自 MIT 的 tikuAdapter, 7 个题库源并发搜题+投票聚合), 无外部程序依赖; 题库源接口属第三方灰色免费/付费接口, 随时可能失效, 单源失败必须静默降级
- 运行时产物均在程序同目录: `session.json`(会话缓存), `tiku.json`(题库, 可写、运行中自动补充), `qingmakiller.log`(日志), `banks.json`(网络题库源配置, **可选**、含付费 token 密钥, 勿提交); 旧版 tikuAdapter 遗留的 `tiku.db`/`tikuadapter.log` 不再是本项目的文件, 勿提交勿引用
...
2. **外部服务异常只降级、不炸轮次**: 网络题库源等外部依赖的任何异常(连接失败/超时/响应为空或畸形)必须捕获并降级为"跳过本题", 参照 `tiku.py#search` 的兜底写法。主循环(main.py)对未处理异常有"连续 3 次即退出"机制, 不允许让可恢复故障触发它。
...
- 包内固定 3 个文件: `QingmaKiller.exe` + `tiku.json` + `LICENSE`(MIT 要求分发副本附带许可证); `banks.json` 含用户密钥, 发行包不携带
...
- 禁止入库或入包的运行时残留: `session.json`、`*.log`、`tiku.db`、`tiku.json.tmp`、`tiku.json.broken`、`banks.json`(含密钥)、`banks.json.broken`、`build/`、`dist/`、`*.spec`(.gitignore 已覆盖)
...
- 付费题库源(enncy/aidian/lemon 及各源付费通道)只有协议层单测覆盖, 无真实 token 做端到端验证; 改动相关代码时以 `tests/test_tiku.py` 的协议断言为准, 免费源可真实联调验收
- 本地全局 Python 环境装有无关大包(matplotlib/numpy/IPython 等)时, PyInstaller 的 hook 会把它们卷进 exe(曾致 exe 从约 10MB 膨胀到 57MB); 本地构建请用干净 venv(仓库惯例目录 `.venv-build`, 已 gitignore), CI 全新 runner 无此问题
